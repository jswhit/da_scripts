...
filename_fg1=sfg_${analdate}_fhr0${ANALINC}_control # 3dvar first guess
filename_fg2=sfg_${analdate}_${charfhr}_ensmean # ens mean first guess
...
export PGM="${execdir}/recenterncio_hybgain.x $filename_fg1 $filename_fg2 $filename_anal1 $filename_anal2 $filename_anal $filenamein $filenameout $alpha $beta -1 $nanals"
errorcode=0
${enkfscripts}/runmpi
status=$?
if [ $status -ne 0 ]; then
