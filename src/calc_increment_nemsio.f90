...
!   language: f95
!
! ifort -O3 -xHOST -I${NEMSIO_INC} -I${NETCDF}/include calc_increment_nemsio.f90
! ${NEMSIO_LIB} ${W3NCO_LIB4} ${BACIO_LIB4} -L${NETCDF}/lib -lnetcdf -lnetcdff
!
!$$$
  ...
  do n=1,nrec
     ...
     if (iret .ne. 0) then
       print *,'error reading rec ',n,trim(filename_fg),iret
       stop
     endif
  ...
  do n=1,nrec
     ...
     if (iret .ne. 0) then
       print *,'error reading rec ',n,trim(filename_anal),iret
       stop
     endif
...
! hydrostatic equation g*dz = -R_d*T_v*dlnp. This is untested code.
  if (inc_delz) then
    ...
    print *,'minval/maxval anal_dz',minval(anal_dz),maxval(anal_dz)
